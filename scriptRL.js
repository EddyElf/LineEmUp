...
// Append squares with drop logic
colors.forEach((color, index) => {
  ...
  // Allow squares to receive drops
  ...
  square.addEventListener("drop", e => {
    ...
    if (draggedImg) {
      ...
      console.log("No dragged image found at drop");
    }
  });
...
function showSuccessMessage() {
  ...
  if (pauseBtn) {
    pauseBtn.disabled = true; // disables the pause button after success
  }
...
function pause() {
  alertSound.play();
  if (isPaused || gameOver) return; // ⛔ Don't pause if already paused or game is over
  isPaused = true;
  clearInterval(countdownInterval);
...
function resumeCountdown() {
  if (!isPaused || gameOver) return; // ⛔ Don't resume if not paused or game is over
  isPaused = false;
...
// Allow dropping images back to imageRow
const imageRow = document.getElementById("imageRow");
...
imageRow.addEventListener("drop", e => {
  ...
  if (draggedImg) {
    ...
    console.log("No dragged image found to drop back");
  }
});
